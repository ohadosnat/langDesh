...
const Quiz = () => {
  ...
  // TODO: useCallback instead of useState.
  const [
    {
      currentChallenge,
      feedbackMsg,
  ...
  const handleCheck = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const usersAnswer = values.answer.toLowerCase().trim();

    // correct
    if (usersAnswer === correctAnswer && !isInputDisabled) {
  ...
  // Skip Challenge
  const skipHandle = (soundEffects: boolean) => {
    if (soundEffects) wrongSound.play();
    ...
    setScore((state) => ({
      ...state,
      skipped: [...state.skipped, challenges[currentChallenge].id],
      streak: 0,
    }));
  };
                  ...
                  <div className="space-y-2 mt-10">
                    {!isInputDisabled && (
                      <Button
                        ...
                        onClickHandle={() =>
                          skipHandle(currentUserDoc!.soundEffects)
                        }
                      >
